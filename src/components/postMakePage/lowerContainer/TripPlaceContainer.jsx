...
import PlusButtonSrc from '../../../../public/assets/images/clipStartPage/ImagePlusButton.svg';
...
const TripPlaceContainer = (props) => {
  ...
  const handlePlaceRemoveFile = (index) => {
    setPlaceImageFiles((prevImageFiles) =>
  ...
  const handleDeleteClick = (location) => {
    const foundIndex = props.locationList.findIndex(
      (foundLocation) =>
        foundLocation.planDay === location.planDay &&
        foundLocation.idx === location.idx,
    );
  ...
  return (
    <PlaceContainer>
      <PlaceHeader>
        ...
        <EditButton
          icon={faXmark}
          onClick={() => handleDeleteClick(props.location)}
        ></EditButton>
      </PlaceHeader>
      <PlaceBodyWrapper>
        <PlaceBody>
          {placeImageFiles.map((file, index) => {
            const url =
              file.img instanceof File
                ? URL.createObjectURL(new Blob([file.img]))
                : null;
            return (
